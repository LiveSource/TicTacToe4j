package tictactoe.client.userInterface;

import junit.framework.TestCase;
import tictactoe.client.userInterface.PlayerMove;

/**
 * Test Class: PlayerMove
 * Performs a move in the game.
 *
 */
public class PlayerMoveTest extends TestCase {

	public void testPlayerMove() { 

		// new PlayerMove();

		assertTrue(true);
	}

/**
 * Test Method: makeMove
 * When the player clicks in a cell, the game draws an O or a X on the game
 grid depending on which player's turn it is.
 *
 */
	public void testMakeMove() { 

		// GameGrid gameGrid;

		//  HTMLTable.Cell cell;

		// PlayerMove.makeMove(gameGridHTMLTable.Cell);

		assertEquals(makeMoveTested, null);
	}

	public void testToggleCurrentPlayer() { 

		// PlayerMove.toggleCurrentPlayer();

		assertEquals(toggleCurrentPlayerTested, null);
	}

}
